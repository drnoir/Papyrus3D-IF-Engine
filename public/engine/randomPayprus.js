...
function getRandomNumberWater() {
  return 10 + Math.random() * 10;
}

const randomSpread = getRandomNumber1();
const randomWaterSpread = getRandomNumberWater();
...
let waterIndexStr = 0;
let waterIndexEnd =1;
let waterWidth = getRandomNumber(10,50);

function randomArrIndexWater(){
  waterIndexStr = getRandomNumber(1, 625);
  waterIndexEnd = waterIndexStr+waterWidth ;
}

// generate random water elm
randomArrIndexWater();
 console.log(waterIndexStr, waterIndexEnd);
...
function generateRandomArray() {
  ...
  for (let i = 0; i < 625; i++) {
    if (i>=waterIndexStr && i<=waterIndexEnd ){
      randomArray.push(paintWater());
    }
    else{
    randomArray.push(getRandomBit());
  }}
  // mapSource = randomArray;
  return randomArray;
}
...
// downloadJSON(randomArray, 'randomArray.json');
...
function randomInit(randomMode){
      randomMode = true;
      return randomMode;
}
...
loadData();
  // DATA LOADING ROUTINES
    async function loadData() {
        ...
        // await downloadJSON(randomArray, 'randomArray.json');
        await createRooms();
...
//     // const res = await fetch(fetchURL)
//     mapSource = await randomArray.json();
// }
...
// EXPORTS 
  export{ generateRandomArray,  randomInit,
